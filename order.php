...
?>
    <h2>
        <center>You are not logged in. <b></b> to place your order.</center>
    </h2>
    <p>
        <center><b>Redirecting you to login page in 5 seconds...</b></center>
    </p>
<?php
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $created = date("Y-m-d");
    $sql = "INSERT INTO `packages` (`id`, `plan`, `Bridegroom`, `Bride`, `Description`, `date`, `location`, `userid`, `created`) VALUES (NULL ,'{$plan}', '{$bridegroom}', '{$bride}', '{$description}', '{$date}', '{$location}',  '{$_SESSION['id']}', '');";
    if (!mysqli_query($conn, $sql)) {
        echo "Server facing error";
        exit();
    }
} else header('location: ./packages.php');
